...
def get_mfrac_var(iso_ratio, iso_ratio_sd, iso_ratio_cov):
    # This function takes three lists as inputs: isotopic ratio of each species, 
    # the standard deviation for each ratio, and the covariance between each ratio and the sum of the ratios.
    # It returns a list containing the variances (sd squared) of the mass fraction for each species.
        ...
        for i in range(len(iso_ratio)):
            d += iso_ratio[i]
            var_d += (iso_ratio_sd[i])**2
        for i in range(len(iso_ratio)):
            var_mfrac.append((iso_ratio[i]/d)**2 * ((iso_ratio_sd[i]/iso_ratio[i])**2 + var_d/(d**2) - 2*iso_ratio_cov[i]/(d*iso_ratio[i])))
        return var_mfrac
...
def to_iso_ratio_var(scaled_ratio, var_scaled_ratio, standard, var_standard, scale = 'delta'):
    # This function takes 4 lists as inputs: the scaled isotopic ratios (delta, epsilon, or mu) and the correpsoding variances, standard ratios
    ...
    # The output is a list containing variances of isotopic ratios.
        ...
        for i in range(len(scaled_ratio)):
            var_iso_ratio.append(var_standard + (scaled_ratio[i]*standard[i]/scales[scale])**2 * (var_scaled_ratio[i]/scaled_ratio[i]**2 + var_standard[i]/standard[i]**2))
        return var_iso_ratio
